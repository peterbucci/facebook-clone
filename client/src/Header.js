...
function Header() {
  ...
  useEffect(() => {
    if (searchInputRef && searchInputRef.current) {
      ...
      searchInputContainer.current.addEventListener('click', handleClick)
      searchInputRef.current.addEventListener('blur', handleBlur)

      return () => {
        searchInputRef.current.removeEventListener('focus', handleFocus)
        searchInputContainer.current.removeEventListener('click', handleClick)
  ...
  return (
    <div className="header">
      <div className={`header__left${searchClicked ? ' search-clicked' : ' search-not-clicked'}`}>   
        <Logo 
          searchClicked={searchClicked}
          className="header__left" 
        />
        <ArrowBackIcon 
          className={`header__left__arrow-back-icon ${searchClicked ? ' clicked' : ' not-clicked'}`} 
          onClick={() => setSearchClicked(false)} 
        />
        <div className={`header__left__absolute-container${searchClicked ? ' clicked' : ' notlicked'}`}>
          <label for="search-input" ref={searchInputContainer} className={`header__left__input${searchClicked ? ' clicked' : ' notlicked'}`}>
          ...
          <div className={`header__left__search-history${searchClicked ? ' search-clicked' : ' search-not-clicked'}`}>
      ...
      <div className="header__center">
        <div className="header__center__container">
          <Link to="/" className={`header__center__option${location.pathname === '/' ? ' active' : ' not-active'}`}>
      ...
      <div className="header__right">
        ...
        <button>
          <svg viewBox="0 0 28 28" height="20" width="20" alt="">
